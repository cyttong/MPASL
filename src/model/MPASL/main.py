...
from data_loader_genem_set import load_data
...
def env_by_genen(args, trn_info):
    if args.top_k == True:
        show_topk_mode = True
    else:
        show_topk_mode = False
    exp_times = 3
    if args.abla_exp == True: exp_times = 3
    for epoch in range(exp_times):
        if epoch == exp_times - 1:
            record_info = True
        else:
            record_info = False
...
if __name__ == "__main__":
    ...
    if args.top_k == True:
        trn_info = Train_info_record_emb_sw_ndcg(args)
    else:
        trn_info = Train_info_record_sw_emb(args)
    env_by_genen(args, trn_info)
