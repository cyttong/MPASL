...
class Eval_score_info:
    def __init__(self):
        self.train_auc_acc_f1_aupr = [0 for _ in range(3)]  #3    4
        self.eval_auc_acc_f1_aupr = [0 for _ in range(3)]  #3
        self.test_auc_acc_f1_aupr = [0 for _ in range(3)]  #3
    ...
    def eval_st_score(self):
        return self.eval_auc_acc_f1_aupr[0]
...
class Train_info_record_sw_emb:
    def __init__(self,args):
        ...
        self.emb_score_aupr = [[0 for _ in range(5)] for _ in range(4)]
        ...
        self.emb_score_aupr_tmp = [[0 for _ in range(5)] for _ in range(4)]
    ...
    def update_cur_train_info(self,args, record_info):    
        ...
        self.max_train_f1 = 0
        self.max_train_aupr = 0
        self.max_eval_auc = 0
        self.max_eval_acc = 0
        self.max_eval_f1 = 0
        self.max_eval_aupr = 0
        self.max_test_auc = 0
        self.max_test_acc = 0
        self.max_test_f1 = 0
        self.max_test_aupr = 0
    ...
    def update_score(self, step, eval_score_info):
        train_auc, train_acc, train_f1,train_aupr = eval_score_info.train_auc_acc_f1_aupr[0], eval_score_info.train_auc_acc_f1_aupr[1], eval_score_info.train_auc_acc_f1_aupr[2], eval_score_info.train_auc_acc_f1_aupr[3]
        eval_auc, eval_acc, eval_f1,eval_aupr = eval_score_info.eval_auc_acc_f1_aupr[0], eval_score_info.eval_auc_acc_f1_aupr[1], eval_score_info.eval_auc_acc_f1_aupr[2], eval_score_info.eval_auc_acc_f1_aupr[3]
        test_auc, test_acc, test_f1 , test_aupr=  eval_score_info.test_auc_acc_f1_aupr[0], eval_score_info.test_auc_acc_f1_aupr[1], eval_score_info.test_auc_acc_f1_aupr[2], eval_score_info.test_auc_acc_f1_aupr[3]

        print('epoch %d  train auc: %.4f acc: %.4f f1: %.4f aupr: %.4f  eval auc: %.4f acc: %.4f f1: %.4f aupr: %.4f test auc: %.4f acc: %.4f f1: %.4f aupr: %.4f'
                  % (step, train_auc, train_acc, train_f1,train_aupr , eval_auc, eval_acc, eval_f1, eval_aupr,test_auc, test_acc, test_f1,test_aupr), end = '\r')
        # train_log = open(self.folder_path, 'a')
        self.logger.info('epoch %d  train auc: %.4f acc: %.4f f1: %.4f aupr: %.4f eval auc: %.4f acc: %.4f f1: %.4f  aupr: %.4f test auc: %.4f acc: %.4f f1: %.4f aupr: %.4f'
                  % (step, train_auc, train_acc, train_f1,train_aupr , eval_auc, eval_acc, eval_f1,eval_aupr, test_auc, test_acc, test_f1,test_aupr))
        # train_log.close()
    ...
    def train_over(self,tags = 0):
        ...
        print('best_score  max_train auc: %.4f acc: %.4f max_f1: %.4f  max_aupr: %.4f  max_eval auc: %.4f acc: %.4f max_f1: %.4f max_aupr: %.4f max_test auc: %.4f acc: %.4f max_f1: %.4f max_aupr: %.4f'
                  % (self.max_train_auc, self.max_train_acc, self.max_train_f1, self.max_train_aupr,self.max_eval_auc, self.max_eval_acc, self.max_eval_f1, self.max_eval_aupr, self.max_test_auc, self.max_test_acc, self.max_test_f1,self.max_test_aupr))
        # train_log = open(self.folder_path_best, 'a')
        self.logger_best.info('best_score max_eval auc: %.4f acc: %.4f max_f1: %.4f max_aupr: %.4f max_test auc: %.4f acc: %.4f max_f1: %.4f max_aupr: %.4f'
                   % (self.max_eval_auc, self.max_eval_acc, self.max_eval_f1, self.max_eval_aupr,self.max_test_auc, self.max_test_acc, self.max_test_f1,self.max_test_aupr))
        # train_log.close()
        ...
        if self.max_eval_auc > self.emb_score_auc_tmp[tr][self.hop][0]:
            ...
            self.emb_score_aupr_tmp[tr][self.hop] = [self.max_eval_aupr, self.max_test_aupr]
    ...
    def record_final_score(self, record_info = False):
        ...
        for tr in range(len(self.emb_score_auc)):
            ...
            emb_score_aupr[tr] = [round(i / self.counter, 6) for i in self.emb_score_aupr[tr]]
            emb_score_recall[tr] = [round(i/self.counter, 6)  for i in self.emb_score_recall[tr]]

        if record_info == True:
            # train_log = open(self.folder_path_best, 'a')
            self.logger_best.info(f"no     auc = {emb_score_auc[0]}, acc = {emb_score_acc[0]}, f1 = {emb_score_f1[0]}, aupr = {emb_score_aupr[0]}")
            # self.logger.info(f"no emb auc = {emb_score_auc[2]}, acc = {emb_score_acc[2]}, f1 = {emb_score_f1[2]}, aupr = {emb_score_aupr[2]}")
